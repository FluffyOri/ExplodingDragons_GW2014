...
function manageTime(){
	...
	var frameSinceLastState = 0;
	var level = getLevel();
	//console.log(level)
	for (var i = 0; i < level.levelDesign.length; i++) {
		//debugger;
		if (level.levelDesign[i].time <= elapsedTime){
			var waveId =level.levelDesign[i].wave;
			var ywave =level.levelDesign[i].y;
			var wave = level.waves[waveId];
			//console.log(level.levelDesign);
			for (var j=0; j<wave.length; j++){
				var splineId = wave[j].spline;
				//console.log(splineId);
				if (wave[j].ennemis ==1){
					world.create.push(new Ennemis(params.ennemis,wave[j].x, ywave+ wave[j].y,level.splines[splineId]))
					}
				}
			//console.log(game.elements.enemy_1,game.level.levelDesign[i].time, game.elapsedTime)
			level.levelDesign.splice(i,1);
			i--;
		}
	}
}		

//addRenderSystem(world.manageTime.prototype);
